...
void emberAfPluginTemperatureSi7053ReadEventHandler(void)
{
  ...
  if (status) {
    emberAfPluginTemperatureSi7053DataReadyCallback(0, false);
  }
  ...
  emberAfPluginTemperatureSi7053DataReadyCallback(tempMilliDegreesC, true);
}
...
uint8_t halTemperatureStartRead(void)
{
  ...
  status = halI2cWriteBytes(SI7503_I2C_ADDR, &cfgMsg, 1);
  if (status) {
