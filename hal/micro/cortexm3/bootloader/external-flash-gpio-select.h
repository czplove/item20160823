...
// Abstract away registers to make access serial controller agnostic
  ...
  #if !defined(EXTERNAL_FLASH_CS_ACTIVE)
  // default to PA7 if it is not already defined
    #define EXTERNAL_FLASH_CS_ACTIVE()     do { GPIO_PACLR = BIT(7); } while (0)
    #define EXTERNAL_FLASH_CS_INACTIVE()   do { GPIO_PASET = BIT(7); } while (0)
    #define EXTERNAL_FLASH_nCS_PIN         PORTA_PIN(7) // PA7
  #endif
