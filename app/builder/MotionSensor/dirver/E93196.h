...
#define DOCI_INIT_IN()                  {GPIO_PBCFGL |=(GPIOCFG_IN  << PB3_CFG_BIT);}  /*!< DOCI初始化为输入 */
#define DOCI_INIT_OUT()                 {GPIO_PBCFGL |=(GPIOCFG_OUT << PB3_CFG_BIT);} /*!< DOCI初始化为输出 */
#define DOCI_ON()                       {GPIO_PBOUT  |=(1 <<PB3_BIT);}        /*!< DOCI输出高电平 */
#define DOCI_OFF()                      {GPIO_PBOUT  &=~(1 <<PB3_BIT);}        /*!< DOCI输出低电平 */
#define DOCI_PULLUP()                   \
{                                                                              \
                                        GPIO_PBCFGL |=(GPIOCFG_IN_PUD << PB3_CFG_BIT);          /*!< DOCI上拉输入 */\
                                        (GPIOOUT_PULLUP   <<PB3_BIT);          \
}
#define DOCI_PULLDOWN()                 \
{                                                                              \
                                        GPIO_PBCFGL |=(GPIOCFG_IN_PUD << PB3_CFG_BIT);          /*!< DOCI下拉输入*/\
                                        (GPIOOUT_PULLDOWN   <<PB3_BIT);        \
                                        }
#define DOCI_3STATE()                   {GPIO_PBCFGL |=(GPIOCFG_IN  << PB3_CFG_BIT);}         /*!< DOCI 3-state输入*/

#define DOCI_ENABLE_INTERRUPT()         {INT_CFGSET = INT_IRQC;}    /*!< 使能DOCI引脚中断*/
#define DOCI_DISABLE_INTERRUPT()        {INT_CFGCLR = INT_IRQC;}    /*!< 禁止DOCI引脚中断*/

/* ------------------E93196 Input SERIN----------------------*/
#define SERIN_INIT_IN()                 {GPIO_PCCFGL |=(GPIOCFG_IN << PC1_CFG_BIT);}       /*!< SERIN初始化为输入 */
#define SERIN_INIT_OUT()                {GPIO_PCCFGL |=(GPIOCFG_OUT<< PC1_CFG_BIT);}       /*!< SERIN初始化为输出 */
#define SERIN_ON()                      {GPIO_PCOUT  |=(1 <<PC1_BIT);}      /*!< SERIN输出高电平 */
#define SERIN_OFF()                     {GPIO_PCOUT  &=~(1 <<PC1_BIT);}      /*!< SERIN输出低电平 */
