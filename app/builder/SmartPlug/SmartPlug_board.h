 ...
 *
 * The EM35x on a Metering Socket BoB has the following example GPIO configuration.
 ...
 * - PA3 - LED_G
 * - PA4 - LED_R
 * - PA5 - Button(IRQC pointed to PA5) SW_1
 * - PA6 - NULL
 * - PA7 - MAX71020_CS
 * - PB0 - Button(IRQA pointed to PB0) SW_0
 * - PB1 - SC1TXD
 * - PB2 - SC1RXD
 * - PB3 - MAX71020_RESET
 * - PB4 - MAX71020_PDS(IRQD pointed to PB4)
 * - PB5 - W25X40CL_CS
 * - PB6 - NULL
 * - PB7 - RELAY_CTRL
 * - PC0 - NULL
 ...
 * connected.
 *
 * \b Note: LEDs 0 and 1 are on the board WL-ZSKMN-M3131-1-P-A-V2.1.2.
 *
 */
//@{
...
enum HalBoardLedPins {
  BOARDLED0 = PORTA_PIN(3),
  BOARDLED1 = PORTA_PIN(4),
  BOARDLED2 = PORTC_PIN(0),
  BOARDLED3 = BOARDLED2,
  BOARD_ACTIVITY_LED  = BOARDLED2,
  BOARD_HEARTBEAT_LED = BOARDLED0,
  BOARDRELAY_CONTROL    = PORTB_PIN(7),
  BOARDW25X40CL_CS      = PORTB_PIN(5),
  BOARDMAX71020_RESET   = PORTB_PIN(3),
  BOARDMAX71020_CS      = PORTA_PIN(7),
};

/** @} END OF LED DEFINITIONS  */

#if !defined(EXTERNAL_FLASH_CS_ACTIVE)
// default to PB5 if it is not already defined
#define EXTERNAL_FLASH_CS_ACTIVE()     do { GPIO_PBCLR = BIT(5); } while (0)
#define EXTERNAL_FLASH_CS_INACTIVE()   do { GPIO_PBSET = BIT(5); } while (0)
#define EXTERNAL_FLASH_nCS_PIN         PORTB_PIN(5) // PB5
#endif

#if !defined(EXTERNAL_MAX71020_CS_ACTIVE)
#define EXTERNAL_MAX71020_CS_ACTIVE()   do { GPIO_PACLR = BIT(7); } while (0)
#define EXTERNAL_MAX71020_CS_INACTIVE() do { GPIO_PASET = BIT(7); } while (0)
#define EXTERNAL_MAX71020_nCS_PIN      PORTB_PIN(7) // PA7
#endif


#if !defined(EXTERNAL_MAX71020_RESERZ_ACTIVE)
#define EXTERNAL_MAX71020_RESERZ_ACTIVE()   do { GPIO_PBCLR = BIT(3); } while (0)
#define EXTERNAL_MAX71020_RESERZ_INACTIVE() do { GPIO_PBSET = BIT(3); } while (0)
#define EXTERNAL_MAX71020_RESERZ_PIN      PORTB_PIN(3) // PB3
#endif
 ...
 */
#define BUTTON0             PORTA_PIN(5)
/**
 * @brief The GPIO input register for BUTTON0.
 */
#define BUTTON0_IN          GPIO_PAIN
/**
 * @brief Point the proper IRQ at the desired pin for BUTTON0.
 * @note For this board, IRQC is pointed at PA5
 */
#define BUTTON0_SEL()       do { GPIO_IRQCSEL = PORTA_PIN(5); } while(0)
/**
...
/**
 * @brief The  interrupt triggering mode for BUTTON0.
 */
#define BUTTON0_INTMOD		0x02//Falling edge triggered.
...
/**
 * @brief The  interrupt triggering mode for BUTTON1.
 */
#define BUTTON1_INTMOD		0x02//Falling edge triggered.

/**
 * @brief The actual GPIO BUTTON2 is connected to.  This define should
 * be used whenever referencing BUTTON2.
 */
#define BUTTON2             PORTB_PIN(4)
/**
 ...
 * @note For this board, IRQD is pointed at PB4
 */
#define BUTTON2_SEL()       do { GPIO_IRQDSEL = PORTB_PIN(4); } while(0)
/**
 * @brief The interrupt service routine for BUTTON2.
 */
#define BUTTON2_ISR         halIrqDIsr
/**
 * @brief The interrupt configuration register for BUTTON2.
 */
#define BUTTON2_INTCFG      GPIO_INTCFGD
/**
 * @brief The interrupt enable bit for BUTTON2.
 */
#define BUTTON2_INT_EN_BIT  INT_IRQD
/**
 * @brief The interrupt flag bit for BUTTON2.
 */
#define BUTTON2_FLAG_BIT    INT_IRQDFLAG
/**
 * @brief The missed interrupt bit for BUTTON2.
 */
#define BUTTON2_MISS_BIT    INT_MISSIRQD
/**
 * @brief The  interrupt triggering mode for BUTTON2.
 */
#define BUTTON2_INTMOD		0x02//Falling edge triggered.
//@} //END OF BUTTON DEFINITIONS

#define GPIO_SENSOR_PIN         0x04
#define GPIO_SENSOR_PORT        HAL_GIC_GPIO_PORTB
#define GPIO_SENSOR_IRQ         HAL_GIC_IRQ_NUMD
...
/** @name LEDs Defines
 * Configure the pin for LEDs.
 *@{
 */
/**
 * @brief Give the LEDs configuration a friendly name.
 */
#define PWRUP_CFG_LED0  GPIOCFG_OUT
#define PWRUP_OUT_LED0  1  /* LED default off */
#define PWRDN_CFG_LED0  GPIOCFG_OUT
#define PWRDN_OUT_LED0  1  /* LED default off */

#define PWRUP_CFG_LED1  GPIOCFG_OUT
#define PWRUP_OUT_LED1  1  /* LED default off */
#define PWRDN_CFG_LED1  GPIOCFG_OUT
#define PWRDN_OUT_LED1  1  /* LED default off */
...
#define PWRUP_CFG_LED3  GPIOCFG_OUT
#define PWRUP_OUT_LED3  1  /* LED default off */
#define PWRDN_CFG_LED3  GPIOCFG_OUT
#define PWRDN_OUT_LED3  1  /* LED default off */
//@} END OF LEDs Configuration Defines
...
#define PWRUP_CFG_SC1_RXD  GPIOCFG_IN_PUD  // Pre-set for UART operation
...
uint16_t gpioCfgPowerUp[6] = {                                                \
        					/* SPI peripheral Pins configure */               \
                            ((GPIOCFG_OUT_ALT    <<PA0_CFG_BIT)|              \
                             (GPIOCFG_IN     <<PA1_CFG_BIT)|              \
                             (GPIOCFG_OUT_ALT    <<PA2_CFG_BIT)|              \
                             /* LED  Pins configure output */                 \
                             (PWRUP_CFG_LED0     <<PA3_CFG_BIT)),             \
                            ((PWRUP_CFG_LED1     <<PA4_CFG_BIT)|              \
                            /* SW_1  Pin configure float input */             \
                             (GPIOCFG_IN_PUD 	 <<PA5_CFG_BIT)|              \
                             (GPIOCFG_IN         <<PA6_CFG_BIT)|              \
                            /* MAX71020_CS Pin configure */                   \
                             (GPIOCFG_OUT        <<PA7_CFG_BIT)),             \
                            /* SW_0  Pin configure float input */             \
                            ((GPIOCFG_IN         <<PB0_CFG_BIT)|              \
                             (GPIOCFG_OUT_ALT    <<PB1_CFG_BIT)| /* SC1TXD  */\
                             (GPIOCFG_IN_PUD     <<PB2_CFG_BIT)| /* SC1RXD  */\
                            /* MAX71020_RESET Pin configure */                \
                             (GPIOCFG_OUT        <<PB3_CFG_BIT)),/* RESET */  \
                            ((GPIOCFG_IN         <<PB4_CFG_BIT)| /* PDS */    \
                            /* W25X40CL_CS Pin configure */                   \
                             (GPIOCFG_OUT        <<PB5_CFG_BIT)|              \
                             (GPIOCFG_IN         <<PB6_CFG_BIT)|              \
                            /* RELAY_CTRL Pin configure */                    \
...
uint8_t gpioOutPowerUp[3] = {                                             \
                           ((0                  <<PA0_BIT)|               \
                            ...
                            (0                  <<PA2_BIT)|               \
                            /* LED  Pins configure output */              \
                            (PWRUP_OUT_LED0     <<PA3_BIT)|               \
                            (PWRUP_OUT_LED1     <<PA4_BIT)|               \
                            (GPIOOUT_PULLUP     <<PA5_BIT)|               \
                            (1                  <<PA6_BIT)|               \
                            /* MAX71020_CS Pin Output */                  \
                            (1                  <<PA7_BIT)),              \
                           ((GPIOOUT_PULLUP     <<PB0_BIT)|               \
                            (1                  <<PB1_BIT)|  /* SC1TXD  */\
                            (GPIOOUT_PULLUP     <<PB2_BIT)|  /* SC1RXD  */\
                            (1                  <<PB3_BIT)|  /* RESET */  \
                            (GPIOOUT_PULLUP     <<PB4_BIT)|  /* PDS */    \
                            (1                  <<PB5_BIT)|               \
                            (GPIOOUT_PULLUP     <<PB6_BIT)|               \
                            /* RELAY_CTRL Pin output */                   \
                            (0                  <<PB7_BIT)),              \
                           ((0                  <<PC0_BIT)|               \
                            (0                  <<PC1_BIT)|               \
                            (0                  <<PC2_BIT)|               \
...
uint16_t gpioCfgPowerDown[6] = {                                              \
                            ...
                            ((GPIOCFG_OUT_ALT    <<PA0_CFG_BIT)|              \
                             ...
                             (GPIOCFG_OUT_ALT    <<PA2_CFG_BIT)|              \
                             /* LED  Pins configure output */                 \
                             (PWRDN_CFG_LED0     <<PA3_CFG_BIT)),             \
                            ((PWRDN_CFG_LED1     <<PA4_CFG_BIT)|              \
                            /* SW_1  Pin configure float input */             \
...
#define WAKE_ON_PA5   true		//BUTTON1
...
#define WAKE_ON_PB4   true		//MAX71020_PDS
