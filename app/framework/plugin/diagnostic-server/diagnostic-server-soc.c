...
bool emberAfReadDiagnosticAttribute(
  ...
  if (emberCounter < EMBER_COUNTER_TYPE_COUNT) {
    if(attributeMetadata->size == 2) {
      ...
      buffer[0] = BYTE_0(emberCounters[emberCounter]);
      buffer[1] = BYTE_1(emberCounters[emberCounter]);
      buffer[2] = BYTE_2(emberCounters[emberCounter]);
      buffer[3] = BYTE_3(emberCounters[emberCounter]);
      return true;
    }
  }
