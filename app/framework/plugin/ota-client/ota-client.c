...
static uint16_t minBlockRequestPeriodSec;
...
{
  minBlockRequestPeriodSec = value;
...
#if defined(ZCL_USING_OTA_BOOTLOAD_CLUSTER_MINIMUM_BLOCK_REQUEST_PERIOD_ATTRIBUTE)
  {
    emberAfWriteAttribute(myEndpoint,
                          ...
                          CLUSTER_MASK_CLIENT,
                          (uint8_t*)&minBlockRequestPeriodSec,
                          ZCL_INT16U_ATTRIBUTE_TYPE);
  }
#endif
}
...
#if defined(ZCL_USING_OTA_BOOTLOAD_CLUSTER_MINIMUM_BLOCK_REQUEST_PERIOD_ATTRIBUTE)
        emberAfPutInt16uInResp(minBlockRequestPeriodSec);
#endif
      }
      break;
...
static EmberAfStatus imageBlockResponseParse(uint8_t* buffer, uint8_t index, uint8_t length)
{
  ...
  uint32_t currentOffset;
  uint8_t dataSize;
  uint16_t minBlockRequestPeriodMs = minBlockRequestPeriodSec * SECONDS_IN_MS;
  ...
  if (buffer != NULL && status == EMBER_ZCL_STATUS_WAIT_FOR_DATA) {
    ...
    if (length >= IMAGE_BLOCK_RESPONSE_WAIT_FOR_DATA_MIN_BLOCK_RESPONSE_LENGTH) {
      ...
      calculatedTimer += temp * SECONDS_IN_MS;
    }
