...
void fetchEmberGpSinkListEntry(EmberGpSinkListEntry *value)
{
  //XXXEZSP
  value->type = fetchInt8u();
...
void appendEmberGpSinkListEntry(EmberGpSinkListEntry *value)
{
  appendInt8u(value->type);
  appendInt16u(value->unicast.sinkNodeId);
  appendInt8uArray(8,value->unicast.sinkEUI);
}
